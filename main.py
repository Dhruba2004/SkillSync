...
def get_course_recommendations_llm(missing_skills, temperature=0.5, max_tokens=512):
    ...
    prompt = f"""
You are an AI Career Advisor. Based on the following skills: {skills_text}, 
return up to 5 real, currently available online courses from Coursera and Udemy.

Each course should have the following fields: 
- title: course name 
- platform: Coursera or Udemy
- Be relevant to the given skills
- link: Exist on Coursera or Udemy (only include valid, working links)
- Be unique (no duplicates)

Output strictly in JSON format only like this:
[
